...
**3. In-memory only — but warmed at startup, not just rebuilt through use.** The routing cache persists to disk; this store deliberately doesn't. Persisting float vectors would mean a new on-disk format, versioning against embedding-model changes, and staleness coupling with `routing_cache.json`, all to save milliseconds on a path that already costs hundreds of them. Instead (v3.54.0), a background thread at startup re-embeds the routing cache's just-loaded persisted `source:` queries — batched into a handful of requests via `llm.embed_batch()`, newest-decided first when there are more queries than slots, live-TTL entries only — restoring the full pre-restart matching ability in seconds without persisting a single vector. The warmup never blocks startup and never degrades anything by failing: a failed batch (embedding backend still booting after a host restart, model not pulled) just logs a warning and leaves the lazy through-use population that was already the normal path. `SEMANTIC_WARMUP_ENABLED=false` turns it off entirely for deployments where those startup warnings would be noise.
