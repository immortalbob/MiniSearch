...
- **[Answer Synthesis](Answer-Synthesis)** — the opt-in `synthesize=true` stage (v3.55.0) that has the local LLM compose a short, grounded answer *from the retrieved material only*, returned in a separate `answer` field alongside the raw `result` — built for the voice pipeline, additive by contract, and grounded-or-silent through a stack of sanity gates (numeric grounding, attribution, honest NOT_IN_SOURCES miss)
